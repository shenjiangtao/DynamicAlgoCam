...
**目标**：为外部激光/云台硬件留出 SDK-neutral 接口契约，沿用 `NioModelBackend` 同款 abstract + factory + self-register 模式。
**预计代码量**：4 个新文件 + 3 处 CMakeLists 修改。
**对应设计文档**：DEVELOPMENT_PLAN.md §3.1, §4 Phase A, §6.1。

### A0. 接口契约冻结（写代码前）
- [x] A0.1 复核 `DEVELOPMENT_PLAN.md` §6.1 `NioActuator` 抽象类签名，由 owner 确认无异议
  - 验证：以书面/issue 形式记录 owner 签字或回复确认
- [x] A0.2 确认 `NioActuatorType` 枚举集合：`NONE / DUMMY / LASER_GENERIC / GIMBAL_GENERIC`（真实设备后两类占位，本 phase 不实现）
  - 验证：枚举列表与契约一致
- [x] A0.3 确认 `NioActuatorConfig` POD 字段：`devicePath / protocolHint / dryRun(=true) / baudRate`
  - 验证：默认 `dryRun=true` 安全默认被明确接受

### A1. 抽象头文件
- [x] A1.1 新增 `app/core/nio_actuator.hpp`：定义 `NioActuatorType` enum、`NioActuatorConfig` struct、`NioActuator` abstract class
  - 验证：头文件仅 `#include "nio_types.hpp"` + STL，不依赖任何 vendor SDK
  - 验证：`grep -n 'ob::\\|rs::\\|libobsensor' app/core/nio_actuator.hpp` 无命中
  - 对应代码：`app/core/nio_actuator.hpp`（新增）

### A2. 工厂 + 自注册表
- [x] A2.1 新增 `app/core/nio_actuator_factory.hpp`：声明 `createActuator()` 与 `registerActuator()` 同款接口
  - 验证：与 `app/core/nio_model_factory.hpp` 同款签名风格
  - 验证：含 `--whole-archive` 静态库链接注意事项注释（自注册模式固有问题）
  - 对应代码：`app/core/nio_actuator_factory.hpp`（新增）
- [x] A2.2 新增 `app/core/nio_actuator_factory.cpp`：mutex + `unordered_map<NioActuatorType, ActuatorCreator>` 实现，与 `nio_model_factory.cpp:1-54` 字节级同款模式
  - 验证：`NioActuatorType::NONE` 输入返回 `nullptr`；重复注册 `NIO_LOG_WARN_S`
  - 对应代码：`app/core/nio_actuator_factory.cpp`（新增）
- [x] A2.3 修改 `app/core/CMakeLists.txt`：将 `nio_actuator_factory.cpp` 加入 `nio_core` 静态库源列表
  - 验证：`cmake --build build --target nio_core` 通过
...
### A3. DUMMY 后端示范
- [x] A3.1 新增 `app/actuator/dummy_actuator.hpp` + `dummy_actuator.cpp`：实现 `DummyActuator : NioActuator`
  - 验证：`load()` 接受 cfg 返回 true；`open()` 打日志返回 true；`aimAt(x,y,z)` 打日志返回 true；`fire(durationMs)` 永远 NO-OP，仅 `NIO_LOG_INFO_S`；`close()` 打日志返回 true；`name()` 返回 `"DUMMY"`
  - 验证：源文件中 `fire()` 实现体不含任何 `::write` / `::open` / 真实 syscall
  - 对应代码：`app/actuator/dummy_actuator.{hpp,cpp}`（新增）
- [x] A3.2 在 `dummy_actuator.cpp` 末尾放置匿名命名空间自注册静态块（`__attribute__((used))`），调用 `registerActuator(NioActuatorType::DUMMY, ...)`；自注册体本身被编译入 `libnio_actuators.a`，但消费者必须 `--whole-archive` 拉入
  - 验证：链接入 `nio_actuators` 后 `nm libnio_actuators.a` 可见 `DummyActuator::fire/open/aimAt/close/name` 与 `registerActuator` 引用符号
- [x] A3.3 新增 `app/actuator/CMakeLists.txt`：生成静态库 `nio_actuators`，PUBLIC 链接 `nio::core`，DAlias `nio::actuators`
  - 验证：`cmake --build build --target nio_actuators` 通过
...
- [x] A5.1 `nio_actuator_factory.hpp` 顶部已加说明注释：消费端链接 `libnio_actuators.a` 时必须使用 `-Wl,--whole-archive ... -Wl,--no-whole-archive`，否则 registrar TU 被链接器丢弃 → `createActuator(DUMMY)` 返回 `nullptr`
  - 验证：注释中 `target_link_libraries(<consumer> PRIVATE "-Wl,--whole-archive" nio::actuators "-Wl,--no-whole-archive")` 模板可见
  - **此约束将直接影响 Phase C4.2 主可执行文件接线**

### A6. 待提交清单
- [x] A6.1 `git add -A` 一次包含以下新增 / 修改文件：
  - `app/core/nio_actuator.hpp`（新）
  - `app/core/nio_actuator_factory.hpp`（新）
  - `app/core/nio_actuator_factory.cpp`（新）
...
- [ ] B0.1 复核反投影公式：`u = det.x + det.w/2`，`v = det.y + det.h/2`，`Z = rawDepthAt(u,v) * depthScale`，`X = (u - intr.cx) * Z / intr.fx`，`Y = (v - intr.cy) * Z / intr.fy`
  - 验证：与 `app/capture/nio_frame_consumer.cpp:139-176` 现成 `PointcloudFrameConsumer` 反投影路径一致（同公式）

### B1. header-only 工具
- [ ] B1.1 新增 `app/core/nio_detection_to_3d.hpp`：声明 `detectionCenterToCamera3D(...)` 函数，含 PRECONDITION 注释块
  - 验证：注释明示 "depth must be D2C-aligned to color frame"
  - 验证：注释明示 `filterHalf=0`/`=2` 含义
  - 验证：函数对深度为 0 / 越界 / NaN 返回 false，输出 X/Y/Z 不变更
  - 对应代码：`app/core/nio_detection_to_3d.hpp`（新增）
- [ ] B1.2 修改 `app/core/CMakeLists.txt`：仅将 header 加入 install 列表，不加入源（header-only）
  - 验证：`grep -c 'nio_detection_to_3d.cpp' app/core/CMakeLists.txt` = 0
...
**目标**：把 `NioModelBackend` + `NioDetectionResult` + `NioKalmanTracker` + Phase B 反投影 + Phase A actuator 串成最小可运行闭环。仅当 `--engage-*` flag 传入时启动。
**预计代码量**：4 个新文件 + 1 处 main CLI 接线 + 文档。
**对应设计文档**：DEVELOPMENT_PLAN.md §4 Phase C, §6.3 状态机图。
**前置依赖**：依赖 A2 (`NioActuator`) 与 B1 (`detectionCenterToCamera3D`)。
...
- [ ] C0.2 确认 `EngagementFrameConsumer` 通过 `FrameConsumer` 接口插入 `session` 的 `frameConsumers_` 链尾 — 复用 `app/capture/nio_capture_session.cpp:157-165` 模式
  - 验证：`CaptureSession::setup/start/startVideoPipeline/stop` 签名零修改

### C1. TargetSelector
- [ ] C1.1 新增 `app/algo/nio_target_selector.hpp`：`SelectorStrategy` enum（`HIGHEST_SCORE / NEAREST_DEPTH / LARGEST_AREA`）；`NioTargetSelector::pick(detections, depthAligned=nullptr)` 返回 `std::optional<NioDetectionResult>`
  - 验证：header 仅 `#include "nio_model.hpp" + <optional> + <vector>`
  - 对应代码：`app/algo/nio_target_selector.hpp`（新增）
- [ ] C1.2 实现 `app/algo/nio_target_selector.cpp`
  - 验证：单元测试 (C5.1) 通过
- [ ] C1.3 新增 `tests/target_selector_test.cpp`：4 个 detections 各按策略选对
  - 验证：通过（或 GTest 缺则 skip）

### C2. TrackBundle
- [ ] C2.1 新增 `app/algo/nio_track_bundle.hpp`：`NioTrackBundle` 持有 `NioKalmanTracker` 一个，加 `init(detection)` / `update(detection, depthAligned, intr, scale)` / `predict()` / `getLast3D()` 接口
  - 验证：`update()` 内部先调 `detectionCenterToCamera3D` 缓存 3D；再调 `NioKalmanTracker::update` 做 2D 平滑
  - 对应代码：`app/algo/nio_track_bundle.{hpp,cpp}`（新增）
- [ ] C2.2 实现 `app/algo/nio_track_bundle.cpp`；构造时 `NioKalmanTracker` 默认参数；不允许 `init()` 重复调用（WARN 并重置）
  - 验证：编入 `nio_algo` 静态库后 `nm | grep NioTrackBundle::update` 有符号

### C3. EngagementLoop 状态机
- [ ] C3.1 新增 `app/algo/nio_engagement_loop.hpp`：`NioEngagementLoop` 类，构造入参 `modelBackend / actuator / selector / trackBundle`；公开 `onFrame(frameSet)` 回调与 `stop()`
  - 验证：依赖 `NioModelBackend` + `NioActuator` + `NioTargetSelector` 抽象指针，不依赖任何 vendor SDK
  - 对应代码：`app/algo/nio_engagement_loop.{hpp,cpp}`（新增）
- [ ] C3.2 实现 `app/algo/nio_engagement_loop.cpp`：状态机 + 每态 `NIO_LOG_INFO_S` trace；`FIRING` 态调用 `actuator->aimAt(X,Y,Z) → fire(durationMs)`；DUMMY actuator 与默认 `dryRun=true` 保证零外部副作用
  - 验证：状态切换均通过 `NIO_LOG_INFO_S` 输出可被日志检索
  - 验证：`aimAt` 入参坐标系为光心 XYZ 米（直接用 TrackBundle `getLast3D()`）
- [ ] C3.3 新增 `app/algo/CMakeLists.txt`：生成静态库 `nio_algo`，链接 `nio_core` + `nio_actuators`（不直接链接 OrbbecSDK / rs_driver）
  - 验证：`cmake --build build --target nio_algo` 通过
- [ ] C3.4 修改根 `app/CMakeLists.txt`：在 `add_subdirectory(actuator)` 之后加 `add_subdirectory(algo)`
  - 验证：全量构建无错

### C4. EngagementFrameConsumer 接线
- [ ] C4.1 新增 `app/algo/nio_engagement_consumer.hpp/.cpp`：`NioEngagementFrameConsumer : FrameConsumer`，`consume(frameSet)` 调 `loop_->onFrame(frameSet)`
  - 验证：基类 `FrameConsumer` 接口完整实现（参考 `app/capture/nio_frame_consumer.hpp:126-195` 中 PointcloudFrameConsumer 同款派生）
...
### D1. README
- [ ] D1.1 修改 `README.md` 架构图 ASCII chart：在 `nio_capture` 下方新增 `nio_actuators` + `nio_algo` 两层注明 "optional, --engage-* flag"
  - 验证：图表与实际目录结构一致
...
- [ ] D3.1 修改 `docs/dynamic_algo_cam/VENDOR_DEVICE_PORTING_MANUAL_CN.md` 末尾追加"如何适配新执行器"章节：与"如何适配新设备" / "如何适配新模型"章节同款体例
  - 验证：包含 NioActuator 子类实现 checklist + registerActuator 自注册 walkthrough + dryRun 安全默认强调
...
| O6 | 3D 坐标 KF 时序滤波（XYZ 而非 bbox） | 现有 `NioKalmanTracker` 单目标验证有效后 | 2026-08-14 |
| O7 | 操作员 HMI / Web 控制台 | 现场试用阶段 | 2026-08-14 |
| O8 | `NioModelType::YOLOV8_PY` 真后端（Python subprocess/IPC 桥） | 决定 Python ↔ C++ 进程边界后 | 2026-08-14 |
...
- 抽象类 + 自注册工厂模式先例：`app/core/nio_model_factory.cpp:1-54`
- `NioDetectionResult` 字段定义：`app/core/nio_model.hpp:45-53`
- `NioIntrinsic` 字段定义：`app/core/nio_types.hpp:257-265`
- `depthScale` 缓存路径：`app/capture/nio_capture_session.cpp:41`
- D2C 自动判定路径：`app/driver/orbbec/nio_ob_device.cpp:281-286`
- `FrameConsumer` 链尾插入先例：`app/capture/nio_capture_session.cpp:157-165`（PCD consumer）
- 反投影公式已有先例：`app/capture/nio_frame_consumer.cpp:139-176`
- `--enable-event-sim` CLI flag 先例：根 `CMakeLists.txt`
- `FrameConsumer` 基类派生先例（pointcloud）：`app/capture/nio_frame_consumer.hpp:126-195`
