...
```
app/
├── core/           # SDK 无关层：DynalgoDevice, DynalgoPipeline, DynalgoFrame, DynalgoTypes
├── driver/         # SDK 适配层（唯一允许引用厂商 SDK 头文件的层）
│   ├── orbbec/     # Orbbec SDK 适配器
│   ├── robosense/  # RoboSense rs_driver 适配器
│   └── dynalgo_driver_factory.hpp/cpp  # 设备发现工厂
├── capture/         # CaptureSession — 仅使用 DynalgoDevice/DynalgoPipeline 接口
├── plugins/opencv/  # OpenCV 工具 — 仅使用 DynalgoFrame/DynalgoFormat
...
## 2. 抽象接口 (app/core/dynalgo_device.hpp)
...
| `getSensorInfo()` | `DynalgoSensorInfo` | 缓存的传感器存在性 + 流配置摘要 |
| `getIntProperty(int)` | int32_t | 读取整数属性（如深度精度） |
| `hasIRSensor()` | bool | 设备是否具有任何 IR 传感器 |
| **`setupPipeline(DynalgoPipeline&)`** | `DynalgoSensorInfo` | **核心方法**：枚举传感器、选择配置、启用流、应用设备 quirks、检测 D2C 模式、返回解析后的传感器信息 |

**框架调用流程**（`app/capture/dynalgo_capture_session.cpp:25-53`）：
...
| 方法 | 返回值 | 用途 |
|------|--------|------|
| `enableStream(DynalgoStreamConfig)` | void | 配置流（类型、宽高、帧率、格式） |
| `disableStream(DynalgoFrameType)` | void | 禁用某流类型 |
| `setAggregateAllTypeFrameRequire(bool)` | void | 帧聚合模式 |
| `setAlignMode(DynalgoAlignMode)` | void | 设置 D2C 对齐模式（HW/SW/NONE） |
| `checkHWD2CSupport(...)` | bool | 检查给定配置下 HW D2C 可行性 |
| `enableFrameSync()` | void | 启用硬件帧同步 |
| **`start(DynalgoVideoCallback)`** | bool | **核心方法**：启动管道；回调接收 `shared_ptr<DynalgoFrameSet>` |
| `startImu(DynalgoImuCallback)` | bool | 启动 IMU 流；回调接收 `vector<DynalgoImuSample>` |
| `stop()` | void | 停止视频管道 |
| `stopImu()` | void | 停止 IMU 管道 |
| `getDevice()` | `shared_ptr<DynalgoDevice>` | 返回底层设备 |
| `isPointCloudDepth()` | bool | 深度为 3D 点云时覆写为 `true`（默认 false） |
| `getAlignMode()` | `DynalgoAlignMode` | 当前对齐模式 |
| `getD2CAlignFilter()` | `shared_ptr<DynalgoD2CAlign>` | 返回 SW 对齐滤镜（默认 nullptr） |
...
| `getDevice(uint32_t)` | `shared_ptr<DynalgoDevice>` | 按索引获取设备 |

### 2.4 DynalgoD2CAlign（SW D2C 场景）

| 方法 | 返回值 | 用途 |
|------|--------|------|
| `process(shared_ptr<void>, DynalgoAlignedFrame&)` | bool | 接收类型擦除的原生 FrameSet，执行对齐，填充输出结构 |

---

## 3. 值类型 (app/core/dynalgo_types.hpp, dynalgo_frame.hpp)
...
辅助函数：
- `nioFormatBpp(DynalgoFormat)` → 每像素字节数（Y8=1, Y16=2, RGB=3, RGBA=4, 其余=0）
- `nioFormatRawSize(DynalgoFormat, w, h)` → 原始缓冲区大小（字节）

### 3.2 DynalgoFrameType
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | DynalgoFrameType | 传感器来源 |
| `format` | DynalgoFormat | 像素格式 |
...
| 方法 | 说明 |
|------|------|
| `getFrame(DynalgoFrameType)` | 获取帧指针（不存在返回 nullptr） |
| `setFrame(DynalgoFrameType, DynalgoFrame)` | 插入帧（转移所有权） |
| `allFrames()` | 遍历所有帧 |
| `nativeFrameSet` | `shared_ptr<void>` — SW D2C 对齐用的不透明原生 FrameSet |

### 3.5 DynalgoSensorInfo

| 字段 | 说明 |
|------|------|
| `hasColor`, `hasDepth`, `hasIR`, `hasIRLeft`, `hasIRRight`, `hasAccel`, `hasGyro` | 传感器存在性 |
| `colorFormat`, `depthFormat`, `irFormat`, `irLeftFormat`, `irRightFormat` | 解析后的 DynalgoFormat |
...
### 阶段 2：实现类型映射 (dynalgo_xyz_.hpp)

必须实现的转换：

| 从 | 到 | 说明 |
|----|----|------|
| 厂商格式枚举 | `DynalgoFormat` | 映射所有厂商像素格式 |
| `DynalgoFormat` | 厂商格式枚举 | 反向映射（可能有损） |
| 厂商帧类型 | `DynalgoFrameType` | 映射传感器类型 |
| `DynalgoFrameType` | 厂商传感器类型 | 用于流启停 |
| 厂商内参 | `DynalgoIntrinsic` | 复制 fx/fy/cx/cy/width/height |
| `DynalgoIntrinsic` | 厂商内参 | 反向映射 |

可能需要的辅助函数：
- `selectBestProfile()` — 若厂商 SDK 支持配置枚举，参考 `dynalgo_ob_adapter.hpp`；若固定配置则硬编码。
- `isLiDARDevice()` — 若厂商支持 LiDAR 传感器，添加到适配器；否则可省略。

### 阶段 3：实现帧转换 (dynalgo_xyz_.hpp)
...
4. **优雅处理缺失帧**：某个流不在厂商 FrameSet 中时跳过 — `DynalgoFrameSet` 默认为空。

### 阶段 4：实现设备类 (dynalgo_xyz_.hpp/cpp)
...
1. 将 `DynalgoPipeline&` 下转为 `XyzPipeline&`。
2. 枚举厂商传感器列表。
3. 对每个传感器类型选择最佳流配置（分辨率、格式、帧率）。
4. 在厂商管道配置上启用选定配置。
5. 用解析后的值填充 `DynalgoSensorInfo`。
6. 应用设备特殊处理（禁用故障流等）。
...
- 当两者就绪时，调用 `tryEmitFrameSet()` 组装并发射 `DynalgoFrameSet`。
...
### 阶段 5：注册到驱动工厂 (dynalgo_driver_factory.cpp)
...
`DiscoveredDevice` 结构定义在 `app/driver/dynalgo_driver_factory.hpp:20-24`：
```cpp
...
**注意**：`nativeFrameSet` 是过渡性设计，存在是因为 `DynalgoD2CAlign::process()` 需要原始 SDK 帧执行厂商特定对齐。若你的厂商对齐可仅基于原始像素数据工作，可在 `process()` 实现中不依赖 `nativeFrameSet`（传 `nullptr`，直接操作 `DynalgoFrame::data`）。

融合决策流程（`app/capture/dynalgo_capture_session.cpp:164-208`）：
...
VideoFrameQueue（有界 SPSC，容量 8）
  ...
  ▼  Video 消费者线程 pop
  │
  ├──► FusionTask (HW: 从 DynalgoFrame 取 color/depth)
...
文件创建门控条件（`app/capture/dynalgo_capture_session.cpp:59-158`）：
...
3. **设备属性 ID**（`getIntProperty(int)`）传递厂商原始 ID。若更多厂商需要，应抽象为 `DynalgoPropertyID` 枚举。
