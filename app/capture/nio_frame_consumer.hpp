...
protected:
    ...
    void dispatchViewIncr_(const NioFrame* frame) {
    ...
    void dispatchViewIncrDepth_(const NioFrame* frame, float depthMinM, float depthMaxM) {
        if (viewerIdx_ >= 0 && viewer_)
    ...
    void incrCount_(NioFrameType type) {
