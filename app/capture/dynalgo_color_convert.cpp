...
//
// dynalgo_color_convert.cpp — Color conversion and text rendering implementation.
//
// Sections:
...
//   3. decodeColorToRGB — universal DynalgoFormat → RGB24 converter
...
bool MjpgDecoderRes::init(int w, int h, DynalgoFormat fmt) {
    pkt = av_packet_alloc();
...
// === Section 3: decodeColorToRGB — universal DynalgoFormat → RGB24 converter ===
...
// chroma misalignment bug described in dynalgo_h264_encoder.cpp.

bool decodeColorToRGB(const uint8_t* data, uint32_t size, DynalgoFormat format, int w, int h, uint8_t* rgbBuf,
                      std::shared_ptr<MjpgDecoderRes> mjpg) {
    if (format == DynalgoFormat::RGB || format == DynalgoFormat::RGB888) {
        memcpy(rgbBuf, data, w * h * 3);
        return true;
    }
    if (format == DynalgoFormat::BGR) {
        for (int i = 0; i < w * h; i++) {
    ...
    if (format == DynalgoFormat::MJPG || format == DynalgoFormat::MJPEG) {
        if (!mjpg->ctx)
            return false;
        ...
        if (!mjpg->swsInitialized) {
            ...
            DYNALGO_LOG_INFO_S("[MJPEG_DEC_FORMAT_CHECK] decodeColorToRGB"
            ...
            if (mjpg->decFrame->data[0]) {
                ...
                DYNALGO_LOG_INFO_S("[MJPEG_DEC_FORMAT_CHECK] fusion sample"
            ...
            DYNALGO_LOG_INFO_S("[MJPEG_DEC_FORMAT_CHECK] fusion sws recreated: srcFmt="
        ...
        if (!fusionRgbLogged && tmpFrame->data[0]) {
            ...
            DYNALGO_LOG_INFO_S("[MJPEG_DEC_FORMAT_CHECK] fusion RGB output sample"
    ...
    if (format == DynalgoFormat::YUYV || format == DynalgoFormat::UYVY || format == DynalgoFormat::YUY2 ||
        format == DynalgoFormat::NV12 || format == DynalgoFormat::NV21 || format == DynalgoFormat::I420) {
        AVPixelFormat srcPixFmt = AV_PIX_FMT_NONE;
        if (format == DynalgoFormat::YUYV)
            srcPixFmt = AV_PIX_FMT_YUYV422;
        else if (format == DynalgoFormat::UYVY)
            srcPixFmt = AV_PIX_FMT_UYVY422;
        else if (format == DynalgoFormat::YUY2)
            srcPixFmt = AV_PIX_FMT_YUYV422;
        else if (format == DynalgoFormat::NV12)
            srcPixFmt = AV_PIX_FMT_NV12;
        else if (format == DynalgoFormat::NV21)
            srcPixFmt = AV_PIX_FMT_NV21;
        else if (format == DynalgoFormat::I420)
        ...
        if (format == DynalgoFormat::YUYV || format == DynalgoFormat::UYVY || format == DynalgoFormat::YUY2) {
            srcStride[0] = w * 2;
        } else if (format == DynalgoFormat::NV12 || format == DynalgoFormat::NV21) {
            srcStride[0] = w;
            srcStride[1] = w;
            srcSlice[1] = data + w * h;
        } else if (format == DynalgoFormat::I420) {
