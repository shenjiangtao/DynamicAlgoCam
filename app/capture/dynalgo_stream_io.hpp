...
//
// dynalgo_stream_io.hpp — Stream I/O utilities: H.264 file writing with buffered
...
//
// All file writes use 4 MB buffered I/O (DYNALGO_FILE_BUF_SIZE) to reduce
...
// Defined in dynalgo_stream_io.cpp (extern + single definition) so the value
// lives in one TU only; avoids `inline constexpr` which would require C++17
// (project standard is C++14).
extern const int DYNALGO_FILE_BUF_SIZE;
...
struct StreamEncoder
{
    ...
    DynalgoFormat srcFormat = DynalgoFormat::UNKNOWN;
    int width = 0;
    int height = 0;
    int fps = 30;
...
struct SensorFiles
{
    ...
    DynalgoFrameCounts frameCounts; // per-type frame counter
    std::mutex countMtx;
};
...
std::shared_ptr<std::ofstream> openBufferedFile(const std::string& path,
                                                ...
                                                int bufSize = DYNALGO_FILE_BUF_SIZE,
...
std::shared_ptr<StreamEncoder> createStreamEncoder(const std::string& filePath, DynalgoFormat format, int w, int h, int fps,
