...
//
// dynalgo_sdl_viewer.cpp — SDL2 live preview implementation for multi-device capture.
//
// Architecture:
...
bool SDLViewer::init() {
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        DYNALGO_LOG_ERROR_S("SDL init failed: " << SDL_GetError());
        return false;
    }
    running_ = true;
    return true;
...
int SDLViewer::addViewerSlot(const std::string& label, DynalgoFormat fmt, int w, int h) {
...
int SDLViewer::addDevice(const std::string& name, const std::string& cameraType, const std::string& serialNumber,
                         bool hasColor, DynalgoFormat colorFmt, int cw, int ch, bool hasDepth, DynalgoFormat depthFmt, int dw,
    ...
    if (hasIR) {
        int idx = addViewerSlot("IR", DynalgoFormat::Y8, irw, irh);
        row.slotIndices.push_back(idx);
        row.irSlot = idx;
    }
    if (hasIRLeft) {
        int idx = addViewerSlot("IR-L", DynalgoFormat::Y8, ilw, ilh);
        row.slotIndices.push_back(idx);
        row.irLeftSlot = idx;
    }
    if (hasIRRight) {
        int idx = addViewerSlot("IR-R", DynalgoFormat::Y8, irw2, irh2);
        row.slotIndices.push_back(idx);
        row.irRightSlot = idx;
    }
    if (hasPoint) {
    ...
    DYNALGO_LOG_INFO_S("SDLViewer: added device " << name << " devIdx=" << devIdx << " slots=" << row.slotIndices.size());
    return devIdx;
}
...
bool SDLViewer::createWindow() {
    ...
    if (SDL_GetCurrentDisplayMode(0, &dm) != 0) {
        DYNALGO_LOG_ERROR_S("SDL_GetCurrentDisplayMode failed: " << SDL_GetError());
        return false;
    }
    ...
    window_ = SDL_CreateWindow("DynamicAlgoCam Monitor", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, winW, winH,
                               SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (!window_) {
        DYNALGO_LOG_ERROR_S("SDL_CreateWindow failed: " << SDL_GetError());
        return false;
    }

    renderer_ = SDL_CreateRenderer(window_, -1, SDL_RENDERER_SOFTWARE);
    if (!renderer_) {
        DYNALGO_LOG_ERROR_S("SDL_CreateRenderer failed: " << SDL_GetError());
    ...
    initialized_ = true;
    DYNALGO_LOG_INFO_S("SDLViewer: window created " << winW << "x" << winH << " slots=" << slots_.size()
...
bool SDLViewer::decodeMjpgSlot(ViewerSlot& slot, const std::vector<uint8_t>& rawCopy, uint32_t rawSz, int w, int h,
    ...
    if (!mjpg->swsInitialized) {
        if (!mjpg->init(w, h, DynalgoFormat::MJPG))
            return false;
    }
    return decodeColorToRGB(rawCopy.data(), rawSz, DynalgoFormat::MJPG, w, h, rgb.data(), mjpg);
}
...
void SDLViewer::decodeSlot(ViewerSlot& slot) {
    ...
    if (slot.format == DynalgoFormat::Y16) {
        ok = decodeY16Slot(slot, rawCopy, rawSz, w, h, rgb);
    } else if (slot.format == DynalgoFormat::POINT) {
        ok = decodePointSlot(slot, rawCopy, rawSz, w, h, rgb);
    } else if (slot.format == DynalgoFormat::Y8) {
        ok = decodeY8Slot(rawCopy, rawSz, w, h, rgb);
    } else if (slot.format == DynalgoFormat::YUYV) {
        ok = decodeYuyvSlot(slot, rawCopy, rawSz, w, h, rgb);
    } else if (slot.format == DynalgoFormat::MJPG || slot.format == DynalgoFormat::MJPEG) {
        ok = decodeMjpgSlot(slot, rawCopy, rawSz, w, h, rgb);
    } else if (slot.format == DynalgoFormat::NV12 || slot.format == DynalgoFormat::NV21 || slot.format == DynalgoFormat::I420 ||
               slot.format == DynalgoFormat::UYVY || slot.format == DynalgoFormat::YUY2 || slot.format == DynalgoFormat::RGB ||
               slot.format == DynalgoFormat::RGB888 || slot.format == DynalgoFormat::BGR || slot.format == DynalgoFormat::RGBA ||
               slot.format == DynalgoFormat::BGRA) {
        if (!slot.mjpgRes) {
...
void SDLViewer::renderLoop() {
    setThreadName("dynalgo-sdl-render");
    while (running_) {
        SDL_Event e;
