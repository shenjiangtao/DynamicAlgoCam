...
PointcloudFrameConsumer::PointcloudFrameConsumer(std::shared_ptr<StreamTask> pcdTask,
                                                 std::shared_ptr<SensorFiles> sensorFiles,
                                                 NioIntrinsic depthIntrinsic, float depthScale)
...
static std::vector<float> backprojectToPointCloud(const uint16_t* y16, int w, int h, const NioIntrinsic& k,
...
void PointcloudFrameConsumer::consume(std::shared_ptr<NioFrameSet> frameSet) {
    auto* driverPoint = frameSet->getFrame(NioFrameType::POINT);
    ...
    if (depthIntrinsic_.fx != 0.0f && depthIntrinsic_.fy != 0.0f && depthScale_ > 0.0f) {
        auto* depthFrame = frameSet->getFrame(NioFrameType::DEPTH);
        if (depthFrame && depthFrame->format == NioFormat::Y16) {
    ...
    if (driverPoint) {
        ...
        if (!ownCloud.empty()) {
            ...
            if (consumed > 0 && driverPts > 0 && driverLayout.srcPointSize >= 12) {
                ...
                NIO_LOG_DEBUG_S("PCD dual-track: driverPts=" << driverPts << " ownPts=" << ownValid
                               << " | sample-mean[driver]=(" << dx << "," << dy << "," << dz
                               << ") vs own=(" << ox << "," << oy << "," << oz << ")");
            } else {
                NIO_LOG_DEBUG_S("PCD dual-track: ownPts=" << ownValid
    ...
    if (!ownCloud.empty() && pcdTask_) {
        ...
        incrCount_(NioFrameType::POINT);
        NIO_LOG_DEBUG_S("PCD fallback: self-computed back-projection fed pcdTask (ownPts=" << ownValid << ")");
    }
}
