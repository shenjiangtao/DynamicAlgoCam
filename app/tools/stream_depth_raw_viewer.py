...
Features:
 ...
 - Controls: Space=play/pause, N=next, P=prev, Q/ESC=quit
"""
import struct, argparse, os, time
import numpy as np
...
def compute_auto_max_depth(path, header, frames_to_sample, exclude_sentinel, scale, percentile):
    # sample first N frames (or fewer if file shorter) and compute percentile of valid z
    ...
    with open(path, 'rb') as f:
        ...
        if auto_max_depth:
            ...
            if suggested is not None:
                ...
                # if user didn't set manual max_depth_m (None or <=0), use suggested
        ...
        if max_depth_m is None or max_depth_m <= 0:
            max_depth_m = 10.0  # fallback

        # Open3D visualizer setup
        vis = o3d.visualization.VisualizerWithKeyCallback()
        vis.create_window(window_name="Depth Stream Viewer (auto)", width=1280, height=720)
        pcd = o3d.geometry.PointCloud()
        vis.add_geometry(pcd)
        render_opt = vis.get_render_option()
        render_opt.point_size = 1.0

        state = {'paused': False, 'frame_idx': 0, 'running': True}
        ...
        while state['running']:
            if not state['paused']:
                ...
                total = depth_ds.size
                num_zero = int(np.sum(depth_ds == 0))
                num_sentinel = int(np.sum(depth_ds >= exclude_sentinel)) if exclude_sentinel is not None else 0
                z_m = depth_ds.astype(np.float32) * scale
                num_over_max = int(np.sum(z_m > max_depth_m))
                if exclude_sentinel is None:
                    mask_valid_all = (depth_ds > 0)
                else:
                    mask_valid_all = (depth_ds > 0) & (depth_ds < exclude_sentinel)
                ...
                pts = depth_to_points(depth_ds, scale, fx_ds, fy_ds, cx_ds, cy_ds, max_depth_m, exclude_sentinel)
