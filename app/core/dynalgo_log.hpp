...
//   DYNALGO_LOG_INFO_S("width=" << w << " height=" << h)  — stream-style logging
//   DYNALGO_LOG_SHUTDOWN()                            — optional explicit close
//
// Log format: "YYYY-MM-DD HH:MM:SS.mmm LEVEL [thread_id] file:line func | msg"
//
// Macro variants:
//   DYNALGO_LOG_INFO(msg)    — msg is a std::string
//   DYNALGO_LOG_INFO_S(msg)  — msg is an ostringstream expression (preferred)
...
public:
    ...
    bool init(const std::string& processName, const std::string& outputDir) {
        ...
        std::call_once(initFlag, [this, &processName, &outputDir]() {
            ...
            if (!logFile_.is_open()) {
                std::cerr << "[DYNALGO_LOG] FATAL: Cannot open log file: " << logFilePath_ << std::endl;
                // If opening fails, we leave initialized_ false so future attempts can retry.
                return;
            }
            initialized_ = true;
            logFile_ << "# DYNALGO Log Start | process=" << processName_ << " | log_file=" << logFilePath_ << "\n";
            logFile_.flush();
        });
    ...
    void shutdown() {
        std::lock_guard<std::mutex> lock(mtx_);
        if (logFile_.is_open()) {
            logFile_ << "# DYNALGO Log End | process=" << processName_ << "\n";
            logFile_.flush();
            logFile_.close();
        }
        initialized_ = false;
...
#define DYNALGO_LOG_INIT(processName, outputDir) ::dynalgo::Logger::instance().init(processName, outputDir)
#define DYNALGO_LOG_SET_LEVEL(level) ::dynalgo::Logger::instance().setLevel(level)
#define DYNALGO_LOG_SHUTDOWN() ::dynalgo::Logger::instance().shutdown()
#define DYNALGO_LOG_FLUSH() ::dynalgo::Logger::instance().flush()
#define DYNALGO_LOG_PATH() ::dynalgo::Logger::instance().getLogFilePath()

#define DYNALGO_LOG_TRACE(msg) ::dynalgo::Logger::instance().log(::dynalgo::LogLevel::TRACE, __FILE__, __LINE__, __func__, msg)
#define DYNALGO_LOG_DEBUG(msg) ::dynalgo::Logger::instance().log(::dynalgo::LogLevel::DEBUG, __FILE__, __LINE__, __func__, msg)
#define DYNALGO_LOG_INFO(msg) ::dynalgo::Logger::instance().log(::dynalgo::LogLevel::INFO, __FILE__, __LINE__, __func__, msg)
#define DYNALGO_LOG_WARN(msg) ::dynalgo::Logger::instance().log(::dynalgo::LogLevel::WARN, __FILE__, __LINE__, __func__, msg)
#define DYNALGO_LOG_ERROR(msg) ::dynalgo::Logger::instance().log(::dynalgo::LogLevel::ERROR, __FILE__, __LINE__, __func__, msg)
#define DYNALGO_LOG_FATAL(msg) ::dynalgo::Logger::instance().log(::dynalgo::LogLevel::FATAL, __FILE__, __LINE__, __func__, msg)
