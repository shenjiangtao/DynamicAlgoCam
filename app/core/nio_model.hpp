...
//
// nio_model.hpp — SDK-neutral model inference abstraction layer.
//
// Defines the contract for pluggable algorithm / inference backends
// (YOLOv8, ONNX Runtime, TensorRT, dummy, ...). Backends live outside
// nio_core (e.g. under app/driver or app/models); this header only states
// the interface. nio_core stays SDK-neutral — no torch / onnx / python
// dependency is added by including this file.
//
// Usage:
//   auto backend = createModelBackend(NioModelType::DUMMY, cfg);
//   if (backend && backend->load("path/to/weights")) {
//       std::vector<NioDetectionResult> out;
...
// Algorithm / inference backend type. Mirrors NioDriverVendor's pattern.
enum class NioModelType {
...
// NioModelFactory (see nio_model_factory.hpp) and live in driver/model code.
class NioModelBackend
{
public:
    virtual ~NioModelBackend() = default;
    ...
    virtual bool infer(const NioFrame& frame, std::vector<NioDetectionResult>& out) = 0;
