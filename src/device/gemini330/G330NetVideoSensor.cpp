...
void G330NetVideoSensor::start(std::shared_ptr<const StreamProfile> sp, FrameCallback callback) {
    if(linkSpeed_ <= G335LE_10M_NET_BAND_WIDTH) {
        THROW_UNSUPPORTED_OPERATION_EXCEPTION(utils::string::to_string()
                                              << "Failed to start " << sensorType_ << " stream: the ethernet link speed is limited to " << linkSpeed_
                                              << "Mb/s! Please check the ethernet connection and reconnect the device.");
    }

    auto backendIter = streamProfileBackendMap_.find(sp);
    if(backendIter == streamProfileBackendMap_.end()) {
        THROW_INVALID_PARAM_EXCEPTION("Can not find backend stream profile for activated stream profile");
    }
