...
void calculate_and_print_frame_rate(const ob_frame *frameset) {
    ...
    ob_error       *error       = NULL;
    const ob_frame *color_frame = NULL;
    ...
    color_frame = ob_frameset_get_frame(frameset, OB_FRAME_COLOR, &error);
    CHECK_OB_ERROR_EXIT(&error);

    // calculate and print color frame rate
    if(color_frame != NULL) {
        uint64_t color_timestamp_current;
        uint64_t duration;

        color_count++;
        // Get timestamp from color frame.
        color_timestamp_current = ob_smpl_get_current_timestamp_ms();
        duration                = color_timestamp_current - color_timestamp_last;
        if(duration > 1000) {  // calculate frame rate every second
            ...
            index = ob_frame_get_index(color_frame, &error);
            CHECK_OB_ERROR_EXIT(&error);

            width = ob_video_frame_get_width(color_frame, &error);
            CHECK_OB_ERROR_EXIT(&error);

            height = ob_video_frame_get_height(color_frame, &error);
            CHECK_OB_ERROR_EXIT(&error);

            frame_rate = color_count / (duration / 1000.0);

            printf("Color frame index: %u, width: %u, height: %u, frame rate: %.2f\n", (uint32_t)index, width, height, frame_rate);
    ...
    depth_frame = ob_frameset_get_frame(frameset, OB_FRAME_DEPTH, &error);
    CHECK_OB_ERROR_EXIT(&error);

    // calculate and print depth frame rate
    if(depth_frame != NULL) {
        uint64_t depth_timestamp_current;
        uint64_t duration;

        depth_count++;
        // Get timestamp from depth frame.
        depth_timestamp_current = ob_smpl_get_current_timestamp_ms();
        duration                = depth_timestamp_current - depth_timestamp_last;
        if(duration > 1000) {  // 1 seconds
            ...
            frame_rate = depth_count / (duration / 1000.0);

            printf("Depth frame index: %u, width: %u, height: %u, frame rate: %.2f\n", (uint32_t)index, width, height, frame_rate);
