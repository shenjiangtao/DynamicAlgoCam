...
class G305SensorStreamStrategy : public ISensorStreamStrategy, public DeviceComponentBase {
public:
    G305SensorStreamStrategy(IDevice *owner);
    virtual ~G305SensorStreamStrategy() noexcept override;
...
private:
    ...
    void validateIRLeftAndColorStream(const std::vector<std::shared_ptr<const StreamProfile>> &profiles);
