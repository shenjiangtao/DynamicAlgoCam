...
PlaybackDepthPostFilterParamsManager::PlaybackDepthPostFilterParamsManager(IDevice *owner, std::shared_ptr<PlaybackDevicePort> port)
    : DeviceComponentBase(owner), port_(port) {
    fetchParamFromDevice();
}

PlaybackDepthPostFilterParamsManager::~PlaybackDepthPostFilterParamsManager() noexcept {}

void PlaybackDepthPostFilterParamsManager::fetchParamFromDevice() {
    std::vector<uint8_t> depthEngineParamData = port_->getRecordedStructData(OB_RAW_DATA_DEPTH_POST_FILTER_PARAMS);
    if(!depthEngineParamData.empty()) {
        uint16_t dataSize = static_cast<uint16_t>(depthEngineParamData.size());
...
bool PlaybackDepthPostFilterParamsManager::calculateChecksum(const uint8_t *data, DepthPostFilterHeader *filterHeader) {
