...
struct PointCloud2_
{
  ...
  _timestamp_type  timestamp_systemusec;
  _timestamp_type  timestamp_globalusec;
...
{
orbbecRosbag::message_operations::Printer< ::sensor_msgs::PointCloud2_<ContainerAllocator> >::stream(s, "", v);
return s;
}
...
struct Printer< ::sensor_msgs::PointCloud2_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::sensor_msgs::PointCloud2_<ContainerAllocator>& v)
  {
    ...
    for(size_t i = 0; i < v.metadata.size(); ++i) {
        s << indent << "  metadata[" << i << "]: ";
        Printer<uint8_t>::stream(s, indent + "  ", v.metadata[i]);
    }
  }
};
