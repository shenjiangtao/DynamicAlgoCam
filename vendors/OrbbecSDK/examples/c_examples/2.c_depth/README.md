...
This is a depth guide to get depth stream and depth image by using the SDK C API.
...
If you are on Windows, you can switch to the directory `build/win_XX/bin` to find the `ob_depth_c.exe`.

If you are on linux, you can switch to the directory `build/linux_XX/bin` to find the `ob_depth_c`.
